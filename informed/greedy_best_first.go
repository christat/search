...
func GreedyBestFirst(origin, target search.HeuristicState) (path map[search.State]search.State, found bool, cost float64) {
	...
	for queue.Size() > 0 {
		...
		found = enqueueBFNeighbors(vertex, target, queue, open, closed, cumulativeCost, path)
		if found {
			break
		}
	}
...
func initBFVariables() (path map[search.State]search.State, cumulativeCost map[search.State]float64, queue *gost.MinPriorityQueue, open, closed map[string]bool) {
...
func enqueueBFNeighbors(vertex, target search.HeuristicState, queue *gost.MinPriorityQueue, open map[string]bool, closed map[string]bool, cumulativeCost map[search.State]float64, path map[search.State]search.State) (found bool) {
	...
	for _, neighbor := range vertex.Neighbors() {
		...
		if !valueSet || cost < lowestCost {
			...
			priority := neighbor.(search.HeuristicState).Heuristic()
